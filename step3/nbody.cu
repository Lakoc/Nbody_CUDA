...
centerOfMass(t_particles p, float *comX, float *comY, float *comZ, float *comW, int *lock, const int N) {
    ...
    float *shared_xw = &shared[0 * bx];
    float *shared_yw = &shared[1 * bx];
    float *shared_zw = &shared[2 * bx];
    float *shared_w = &shared[3 * bx];
    ...
    for (; global_id < N; global_id += blockDim.x * gridDim.x) {
        weight = p.weight[global_id];
        temp_w += weight;
        temp_xw += p.pos_x[global_id] * weight;
        temp_yw += p.pos_y[global_id] * weight;
        temp_zw += p.pos_z[global_id] * weight;
    }

    shared_xw[shared_index] = temp_xw;
    shared_yw[shared_index] = temp_yw;
    shared_zw[shared_index] = temp_zw;
    shared_w[shared_index] = temp_w;
    __syncthreads();

    for (unsigned stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (shared_index < stride) {
            shared_xw[shared_index] += shared_xw[shared_index + stride];
            shared_yw[shared_index] += shared_yw[shared_index + stride];
            shared_zw[shared_index] += shared_zw[shared_index + stride];
            shared_w[shared_index] += shared_w[shared_index + stride];
        }
    ...
    if (threadIdx.x == 0) {
        float w = shared_w[0];
        float x = shared_xw[0] / w;
        float y = shared_yw[0] / w;
        float z = shared_zw[0] / w;


        while (atomicExch(lock, 1) != 0);
        const float dx = x - *comX;
        const float dy = y - *comY;
        const float dz = z - *comZ;

        // Calculate weight ratio only if at least one particle isn't massless
        const float dw = ((w + *comW) > 0.0f)
                         ? (w / (w + *comW)) : 0.0f;
